...
public class FacilityCalendarView extends LinearLayout implements ScrollableConst, TouchableConst {
    ...
    private DayView previousDayView;
...
//    private int mEndOfWeek;

    //SelectedDay
    private int SelectedViewID = 0;
    private int SelectedDayY = 0;
    private int SelectedDayM = 0;
     ...
     */
    public FacilityCalendarView(Context context) {
        this(context, null);
     ...
     */
    public FacilityCalendarView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
...
//        mEndOfWeek = stylesArray[10];
    ...
    private void init() {
        ...
        mView = LayoutInflater.from(mContext).inflate(R.layout.calendar_with_title, this, true);
    ...
    private void initTitleLayout() {
        ...
        RobotoBoldTextView dateTitle = (RobotoBoldTextView) mView.findViewById(R.id.dateTitle);
...
//        dateTitle.setTextColor(mCalendarTitleTextColor);
    ...
    private void initWeekLayout() {
        ...
        for (int i = 1; i < weekDaysArray.length; i++) {
            ...
            //Set display week column
//            if (dayOfWeek.getText().toString().equals("SUN")) {
//                dayOfWeek.setTextColor(mEndOfWeek);
//            }
        }
     ...
     */
    //* Important, set current month
    private void setDaysInCalendar() {
        Currcalendar = Calendar.getInstance(getLocale());
        Currcalendar.setTime(mCalendar.getTime());
        Currcalendar.set(Calendar.DAY_OF_MONTH, 1);
        Currcalendar.setFirstDayOfWeek(mFirstDayOfWeek);
        int firstDayOfMonth = Currcalendar.get(Calendar.DAY_OF_WEEK);

        // Calculate dayOfMonthIndex
        int dayOfMonthIndex = CalendarUtil.getWeekIndex(firstDayOfMonth, Currcalendar);
        int actualMaximum = Currcalendar.getActualMaximum(Calendar.DAY_OF_MONTH);

        final Calendar startCalendar = (Calendar) Currcalendar.clone();
        //Add required number of days
        startCalendar.add(Calendar.DATE, -(dayOfMonthIndex - 1));
        int monthEndIndex = 42 - (actualMaximum + dayOfMonthIndex - 1);

        //Month Year
        int cMonth = Currcalendar.get(Calendar.MONTH);
        int cYear = Currcalendar.get(Calendar.YEAR);

        DayView dayView;
        ViewGroup dayOfMonthContainer;

//        Calendar c = Calendar.getInstance();
//        c.setTime(getDayBooked().get(0));
//        int bookedday = c.get(Calendar.DAY_OF_MONTH);
        for (int i = 1; i < 43; i++) {
            ...
            if (CalendarUtil.isSameMonth(Currcalendar, startCalendar)) {
                dayView.setBackgroundColor(mDisabledDayBackgroundColor);
                dayView.setTextColor(mDisabledDayTextColor);
                int y = i;
//                if ((y % 7) == 0) {
//                    dayView.setTextColor(mEndOfWeek);
//                }
                if (!getSaveDate().isEmpty()) {
                    for (int j = 0; j < getSaveDate().size(); j++) {
                        int selectedDateMonth = 0;
                        int selectedDateYear = 0;
                        int selectedDateDay = 0;
                        Calendar calendar = Calendar.getInstance();
                        try {
                            Date d = getSaveDate().get(j);
                            calendar.setTime(d);
                            selectedDateMonth = calendar.get(Calendar.MONTH);
                            selectedDateYear = calendar.get(Calendar.YEAR);
                            selectedDateDay = calendar.get(Calendar.DAY_OF_MONTH);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        if (selectedDateYear == Currcalendar.get(Calendar.YEAR) && selectedDateMonth == Currcalendar.get(Calendar.MONTH) && (selectedDateDay) == dayView.getDayViewDay()) {
                ...
                // check current view match selectedDay view
                String tag = (String)dayOfMonthContainer.getTag();
                tag = tag.substring(mContext.getString(R.string.day_of_month_container).length(), tag.length());
                if(Integer.parseInt(tag) == SelectedViewID && cMonth ==SelectedDayM && cYear == SelectedDayY){
                    dayView.setBackgroundColor(mSelectedDayBackground);
...
//                dayView.setTextColor(mEndOfWeek);
    ...
    public void setDateAsSelected(Date currentDate, Date previousSelectedDay) {
        Calendar c1 = Calendar.getInstance();
        c1.setTime(currentDate);
        int SelectedDayMonth = c1.get(Calendar.MONTH);
        int SelectedDayYear = c1.get(Calendar.YEAR);
        c1.setTime(previousSelectedDay);
        int previousMonth = c1.get(Calendar.MONTH);
        int previousYear = c1.get(Calendar.YEAR);
        ...
        SelectedDayM = SelectedDayMonth;
        SelectedDayY = SelectedDayYear;
        ...
        final Calendar calendar = Calendar.getInstance(getLocale());
        calendar.setTime(previousSelectedDay);

        if (SelectedDayMonth != previousMonth) {
            if (previousDayView != null) {
                previousDayView.setBackgroundColor(mDisabledDayBackgroundColor);
                previousDayView.setTextColor(mDisabledDayTextColor);
                // return previous view text color
                String pdvTag = previousDayView.getTag().toString();
                pdvTag = pdvTag.substring(mContext.getString(R.string.day_of_month_text).length(), pdvTag.length());
//                if((Integer.parseInt(pdvTag)%7) ==0){
//                    previousDayView.setTextColor(mEndOfWeek);
//                }
            }
        }
        previousDayView = view;
    }
...
//            DayView dayView = (DayView) view;
            ...
            //store tag to match the selected day
            SelectedViewID = Integer.parseInt(tagId);
//            if ((SelectedViewID % 7) == 0) {
//                dayView.setTextColor(mEndOfWeek);
//            }
            ...
            //*Select new Day event , first is current day time, second is previous clicked day time
            setDateAsSelected(calendar.getTime(), previousDay);
    ...
    public ArrayList<Date> getSaveDate() {
        return SaveDate;
    }

    public static void setSaveDate(ArrayList<Date> saveDate) {
        SaveDate = saveDate;
    }

    public static ArrayList<Date> SaveDate = new ArrayList<>();
    ...
    public void setCurrentDay(@NonNull Date todayDate) {
        ...
        if (CalendarUtil.isToday(calendar)) {
            ...
            dayOfMonth.setTextColor(mSelectedDayTextColor);
            dayOfMonth.setBackgroundColor(mSelectedDayBackground);
        }
    }

    public void setCurrentDayDisplayGone(@NonNull Date todayDate) {
        final Calendar calendar = Calendar.getInstance(getLocale());
        calendar.setTime(todayDate);
        ...
        if (CalendarUtil.isToday(calendar)) {
            ...
            dayOfMonth.setTextColor(mDayOfWeekTextColor);
            dayOfMonth.setBackgroundColor(mCalendarBackgroundColor);
        }
    }
}
